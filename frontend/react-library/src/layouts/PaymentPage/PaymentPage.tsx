...
export const PaymentPage = () => {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      if (authState && authState.isAuthenticated) {
        const url = `${process.env.REACT_APP_BASE_URL}/payments/search/findByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
        ...
        if (!paymentResponse.ok) {
          throw new Error(
            "Something went wrong while getting payment request!"
          );
        }
  ...
  async function checkout() {
    if (!stripe || !elements || !elements.getElement) return;
    ...
    const stripeRes = await fetch(url, requestOptions);
    if (!stripeRes.ok) {
        setHttpError(true);
        setSubmitDisabled(false);
        console.log("Something went wrong in our API payment-intent");
        return;
    }

    const resJson = await stripeRes.json();
    stripe.confirmCardPayment(
        ...
        if (result.error) {
            ...
            if (!stripeRes.ok) {
                setHttpError(true);
                setSubmitDisabled(false);
                console.log("Something went wrong in our API payment-complete");
                return;
            }
            setFees(0);
            setSubmitDisabled(false);
